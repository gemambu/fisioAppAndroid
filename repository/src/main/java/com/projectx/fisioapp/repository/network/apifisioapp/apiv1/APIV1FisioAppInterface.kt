...
internal interface APIV1FisioAppInterface {
    ...
    @GET(BuildConfig.FISIOAPP_APPOINTMENTS_PROFESSIONAL_SERVER_PATH + "?dateFrom={dateFrom}&dateTo={dateTo}")
    fun doGetAppointmentsForDate(@Header("x-access-token") token: String, @Path("dateFrom") dateFrom: String, @Path("dateTo") dateTo: String): Call<GetAppointmentsResponse>
